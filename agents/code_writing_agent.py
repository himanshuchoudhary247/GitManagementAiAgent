...
class CodeWritingAgent:
    ...
    def write_code_changes(self, code_changes):
        try:
            for change in code_changes:
                ...
                if not action or not file or not code:
                ...
                if action.lower() == 'add':
                    self.add_function(file_path, code)
                elif action.lower() == 'update':
                    self.update_function(file_path, code)
                else:
    ...
    def add_function(self, file_path, code):
        try:
            ...
            if os.path.exists(file_path):
                ...
                with open(file_path, 'w') as f:
                    f.write(code)
                logging.info(f"Created new file '{file_path}' with the new function.")
                print(f"Created new file '{file_path}' with the new function.")
    ...
    def update_function(self, file_path, code):
        try:
            ...
            # Assuming the code contains the entire function definition
            lines = content.split('\n')
            start_line, end_line = find_function_definitions(content, self.extract_function_name(code))

            if start_line is None or end_line is None:
                logging.error(f"Function '{self.extract_function_name(code)}' not found in '{file_path}'.")
                print(f"Error: Function '{self.extract_function_name(code)}' not found in '{file_path}'.")
                return
            ...
            logging.info(f"Updated function '{self.extract_function_name(code)}' in '{file_path}'.")
            print(f"Updated function '{self.extract_function_name(code)}' in '{file_path}'.")
