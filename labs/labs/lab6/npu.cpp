    ...
    : sc_module(name), m_enable_start(false)
{
    ...
    // Register the new method for micro start control.
            ...
            if (offset == CTRL_OFFSET) {
                ...
                if (ctrl_val & 0x1) {
                    if (m_enable_start) {
                        ...
                        std::cout << "[" << name() << "] write: CTRL SFR trigger ignored because i_start_micro is 0 at time " 
                                  << sc_time_stamp() << std::endl;
                    }
                }
            }
...
void NPU::end_of_elaboration() {
    // At end of elaboration, drive o_interrupt to INT_IDLE.
    o_interrupt.write(INT_IDLE);
