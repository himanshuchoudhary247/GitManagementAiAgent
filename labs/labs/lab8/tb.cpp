...
tb::tb(sc_module_name name)
    : sc_module(name),
      fifo_ifm("fifo_ifm", 10),
      fifo_wt("fifo_wt", 10),
      fifo_ofm("fifo_ofm", 10)
{
    // Instantiate NPU.
    ...
    mem_ifm = new memory("mem_ifm");
    mem_wt  = new memory("mem_wt");
    mem_ofm = new memory("mem_ofm");
    ...
    // Bind memory reset ports.
    mem_ifm->i_reset(reset_sig);
    mem_wt->i_reset(reset_sig);
    mem_ofm->i_reset(reset_sig);

    // Bind testbench external ports.
    o_reset.bind(reset_sig);
    o_start_micro.bind(start_micro_sig);
    i_interrupt.bind(intr_sig);
...
void tb::basic_test() {
    // Program dimensions: high 16 bits = height, low 16 bits = width.
    ...
    std::cout << "[" << name() << "] basic_test: Triggered NPU operation at " 
    ...
    std::cout << "[" << name() << "] basic_test: STATUS=0x" << std::hex << stVal << std::dec
