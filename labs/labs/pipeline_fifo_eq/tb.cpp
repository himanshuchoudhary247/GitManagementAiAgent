...
void TB::producer() {
  ...
  wait(30, SC_NS);  // Second data arrives around t = 40 ns
  ...
  for (int i = 0; i < 3; i++) {
    ...
    cout << "[TB] Producer: Writing burst data " << data << " at " << sc_time_stamp() << endl;
...
void TB::consumer() {
  // Initially, the consumer is ready.
  ...
  while (true) {
    ...
    if (sig_out_valid.read()) {
      ...
      // If the data is 202, simulate a stall for 3 cycles (30 ns).
      if (data == 202) {
