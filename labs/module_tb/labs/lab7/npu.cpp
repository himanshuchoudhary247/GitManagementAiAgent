    ...
    if (it != sfr_map.end()) {
        if (offset == INT_CLEAR_OFFSET) {
            ...
            if (offset == CTRL_OFFSET) {
                ...
                if (ctrl_val & 0x1) {
                    if (m_enable_start) {
                        // Spawn a dynamic process to run start_operation using a lambda.
                        sc_core::sc_spawn([this]() { this->start_operation(); }, "dynamic_start_operation");
                    } else {
...
void NPU::handle_reset() {
    if (i_reset.read() == true) {
        ...
        for (auto it = sfr_map.begin(); it != sfr_map.end(); ++it) {
            it->second->reset();
        }
        o_interrupt.write(INT_IDLE);
    }
...
void NPU::end_of_elaboration() {
    ...
    sc_core::sc_spawn([this]() { this->dummy_thread(); }, "dummy_thread_dynamic");
}
