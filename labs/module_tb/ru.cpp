...
/* Thin wrapper – forwards all ports to the funccore. */
ru::ru(sc_core::sc_module_name name, int id_)
    ...
    , funccore  ("funccore")
{
    funccore.clk(clk);       funccore.reset(reset);
    ...
    funccore.set_Id(id_);  id = id_;
}

void ru::set_id(int set_id) { id = set_id; funccore.set_Id(set_id); }

/* Optional stand‑alone factory (unused by TB) */
void instantiate_RU() { static ru demo("ru_demo",0); }
